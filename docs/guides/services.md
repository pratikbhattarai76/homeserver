# Services 
Each service runs in its own Docker container and is deployed via Docker compose. Services are grouped by purpose. All og them sit on the `docker-proxy` network internally, excepr `cloudflared` which sits on `docker-cloudflared`. Nginx Proxy Manager (NPM) sits in the two networks.
...
Self-hosted file storage and personal cloud. The app container runs Nextcloud on Apache, the db conatiner runs MariaDB. They are chained so that the app waits for the database to be halthy before starting. The 1TB hdd is mounted into the app  at `/external/storage` and exposed through Nextcloud's External Storage feature.
...
### uptime-kuma
Service uptime monitoring. Probes each public service over HTTP and tracks response time and availibility over time. Independent of the Prometheus stack so that monitoring still works if Prometheus or Grafama goes down
